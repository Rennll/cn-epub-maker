...
from .transforms import JunkCleaner, OpenCCTransformer, PunctuationTransformer, TransformAudit, TransformPipeline, TransformationError
from .validator import validate_book, validate_epub


def _run_transformations(lines: list[str], policy: TransformationPolicy, *, full_source: bool) -> tuple[list[str], list[TransformAudit]]:
    if full_source:
        return lines, []
    transformers = [JunkCleaner(rules=list(policy.junk_cleaner.rules))]
    if policy.opencc.enabled: transformers.append(OpenCCTransformer(profile=policy.opencc.profile))
    if policy.punctuation_enabled: transformers.append(PunctuationTransformer())
...
def _print_transform_audit(audit: list[TransformAudit]) -> None:
    for stage in audit:
        if stage.name == "opencc": print(f"Transformation: OpenCC ({stage.metadata.get('profile', 'unknown')})")
        elif stage.name == "punctuation": print("Transformation: Punctuation")
        elif stage.name == "junk_cleaner": print("Transformation: Junk Cleaner")
        for warning in stage.warnings: print(f"WARNING: {stage.name}: {warning}", file=sys.stderr)


def execute(request: ConversionRequest, *, keep_intermediate: bool = False, intermediate: str | None = None) -> int:
    try:
        ...
        if report.errors:
            ...
            return 2
        print(f"Encoding: {encoding}"); print(f"Book: {result.book.title}"); print(f"Author: {result.book.author}")
        print(f"Volumes: {len(result.book.volumes)}"); print(f"Chapters: {result.book.chapter_count}"); print(f"Paragraphs: {result.book.paragraph_count}")
        transformation_warning_count = sum(len(stage.warnings) for stage in audit)
        print(f"Warnings: {len(result.warnings) + transformation_warning_count}"); _print_transform_audit(audit)
        for warning in result.warnings:
            where = f" at line {warning.line}" if warning.line else ""
            print(f"WARNING: {warning.message}{where}", file=sys.stderr)
        if keep_intermediate:
            intermediate_path = Path(intermediate or Path(request.source).with_suffix("").name + ".intermediate")
        ...
        if epub_errors:
            for error in epub_errors: print(f"ERROR: {error}", file=sys.stderr)
            return 3
        print(f"EPUB: {request.destination}"); return 0
    except TransformationError as exc:
        print(f"ERROR: {exc}", file=sys.stderr); return 1
    except FileNotFoundError as exc:
        print(f"ERROR: required executable or file not found: {exc}", file=sys.stderr); return 1
