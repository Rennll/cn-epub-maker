...
def test_repeated_line_becomes_detection_group_with_exact_rule():
    document = build_physical_document(["第一章", "本章字數：1234", "正文", "本章字數：5678", "下一章"])
...
def test_repeated_numbered_headings_are_detected_but_not_qualified():
    ...
    group = next(group for group in detect_document(document) if group.pattern == "第<number>章")
...
def test_generic_numeric_pattern_without_format_marker_is_not_qualified():
    ...
    group = next(group for group in detect_document(document) if group.pattern == "<number>")
    assert group.qualified is False
    assert group.suggested_rule is None
...
def test_identical_repeated_line_uses_exact_rule():
    ...
    group = next(group for group in detect_document(document) if group.pattern == "本章完")
    assert group.suggested_rule.matcher == "exact"


def test_single_occurrence_is_not_a_detection_group():
    assert detect_document(build_physical_document(["正文", "本章字數：1234", "其他"])) == ()


def test_repeated_identical_blocks_produce_a_block_candidate():
    groups = detect_document(build_physical_document(["A", "B", "", "A", "B"]))
    group = next(group for group in groups if group.scope == "block")
